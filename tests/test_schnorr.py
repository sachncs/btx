...
def test_verify_schnorr_bad_pubkey_length() -> None:
    assert not verify_schnorr_signature(b"\x00" * 31, b"\x00" * 64, b"\x00" * 32)
    assert not verify_schnorr_signature(b"\x00" * 33, b"\x00" * 64, b"\x00" * 32)


def test_verify_schnorr_bad_sig_length() -> None:
    assert not verify_schnorr_signature(b"\x00" * 32, b"\x00" * 63, b"\x00" * 32)
    assert not verify_schnorr_signature(b"\x00" * 32, b"\x00" * 65, b"\x00" * 32)


def test_verify_schnorr_bad_msg_length() -> None:
    assert not verify_schnorr_signature(b"\x00" * 32, b"\x00" * 64, b"\x00" * 31)
    assert not verify_schnorr_signature(b"\x00" * 32, b"\x00" * 64, b"\x00" * 33)
...
def test_verify_schnorr_r_geq_p() -> None:
    ...
    msg = sha256(b"hello")
    assert not verify_schnorr_signature(pubkey_bytes, sig, msg)
...
def test_verify_valid_bip340_vector() -> None:
    ...
    assert verify_schnorr_signature(pubkey, sig, msg)
