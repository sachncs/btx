# Copyright (c) 2026 secp contributors
# SPDX-License-Identifier: MIT
"""Low-level binary encodings and hash primitives for Bitcoin.

A small, dependency-free toolkit covering every binary encoding needed
to read and write Bitcoin wire-format structures:

- :mod:`btx.encoding.hex` – :func:`encode_hex` / :func:`decode_hex`.
- :mod:`btx.encoding.varint` – Bitcoin variable-length integer
  (``compact size``) codec.
- :mod:`btx.encoding.der` – DER signature codec used by ECDSA.
- :mod:`btx.encoding.sec` – SEC-1 public-key codec.
- :mod:`btx.encoding.hasher` – SHA-256, double-SHA-256, HASH-160,
  and BIP-340 tagged hash.

None of these helpers perform any I/O or hold any state; they are
suitable for use in hot paths.
"""

from btx.encoding.der import decode_der, encode_der
from btx.encoding.hasher import hash160, hash256, sha256, tagged_hash
from btx.encoding.hex import decode_hex, encode_hex
from btx.encoding.sec import parse_sec, serialize_sec
from btx.encoding.varint import decode_varint, encode_varint

__all__ = [
    "decode_der",
    "decode_hex",
    "decode_varint",
    "encode_der",
    "encode_hex",
    "encode_varint",
    "hash160",
    "hash256",
    "parse_sec",
    "serialize_sec",
    "sha256",
    "tagged_hash",
]
